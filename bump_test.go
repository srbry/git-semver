...
var _ = Describe("#Bump", func() {
	var _ = DescribeTable("Calculate version bumping rule",
		...
		Entry("Features with scope should be minor", []string{"feat(scope): Example"}, git_semver.Minor),
		...
		Entry("Deps should be patch", []string{"deps: Example"}, git_semver.Patch),
		Entry("Build should be none", []string{"build: Example"}, git_semver.None),
		Entry("Deps with scope should be patch", []string{"deps(scope): Example"}, git_semver.Patch),
		Entry("Build with scope should be none", []string{"build(scope): Example"}, git_semver.None),
		...
		Entry("Breaking changes should be major including scope", []string{"refactor(scope): Example\n\nBREAKING CHANGE: breaks everything"}, git_semver.Major),
		...
		Entry("The highest possible version should be returned, major, patch, minor with scope", []string{"feat(scope)!: example", "fix: example", "feat: Example"}, git_semver.Major),
